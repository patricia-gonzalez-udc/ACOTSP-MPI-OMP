...
long int max_iters;         /* maximum number of iterations in one try */
long int seed;
...
FILE *summary, *parallel, *report, *comp_report, *stat_report, *cc_report;
...
*/
{
    if (mpi_id == 0) printf("best %ld, iteration: %ld, time %.2f\n",best_global_tour_length,iteration,elapsed_time( REAL));
    if (comp_report) 
        fprintf(comp_report,
...
*/
{
  ...
  if (!quiet_flag) {
    ...
    /*   These buffers would be useful if we want to see the particular results of each colony */
      ...
      stat_report = NULL;
/*
      sprintf(temp_buffer,"parallel.%s.threads.%d.mpi.%d",instance.name,omp_get_max_threads(),mpi_id);
      TRACE ( printf("%s\n",temp_buffer); )
          parallel = fopen(temp_buffer, "w");
      sprintf(temp_buffer,"best.%s.threads.%d.mpi.%d",instance.name,omp_get_max_threads(),mpi_id);
      TRACE ( printf("%s\n",temp_buffer); )
          report = fopen(temp_buffer, "w");
      sprintf(temp_buffer,"cmp.%s.threads.%d.mpi.%d",instance.name,omp_get_max_threads(),mpi_id);
      TRACE ( printf("%s\n",temp_buffer); )
          comp_report = fopen(temp_buffer, "w");
      sprintf(temp_buffer,"stat.%s.threads.%d.mpi.%d",instance.name,omp_get_max_threads(),mpi_id);
      TRACE ( printf("%s\n",temp_buffer); )
