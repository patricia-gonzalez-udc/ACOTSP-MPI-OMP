...
long int **global_tour;
long int best_global_tour_length;
...
double   trail_0;         /* initial pheromone level in BWAS */
...
*/
{
    ...
    /* memory for tours arriving from other colonies */
    if ((best_global_tour = malloc(sizeof(long int)*(n+1)))==NULL) {
	printf("Best_global_tour. Out of memory, exit.");
	exit(1);
    }
    if ((global_tour = (long int **)malloc(sizeof(long int)*NPROC))==NULL) {
	printf("global_tour. Out of memory, exit.");
	exit(1);
    }
    for ( i=0 ; i<NPROC ; i++ )
         global_tour[i] = (long int *)malloc(sizeof(long int)*(n+1));
...
{ 
    ...
    if ((prob_ptr = malloc(sizeof(double) * (nn_ants + 1))) == NULL) {
	printf("Out of memory in prob_ptr, exit.");
	exit(1);
    }
    /* Ensures that we do not run over the last element in the random wheel.  */
